...
for i, row in enumerate(grid):
    for j, el in enumerate(row):
        # check horizontals
        if j < (len(row) - 3):
            product = el * row[j+1] * row[j+2] * row[j+3]
            if product > answer:
                answer = product

        # check verticals and diagonals
        if i < (len(grid) - 3):
            product = el * grid[i+1][j] * grid[i+2][j] * grid[i+3][j]
            if product > answer:
                answer = product

            # check left diagonals (downward)
            if j > 2:
                product = el * grid[i+1][j-1] * grid[i+2][j-2] * grid[i+3][j-3]
                if product > answer:
                    answer = product

            #check right diagonals (downward)
            if j < (len(row) - 3):
                product = el * grid[i+1][j+1] * grid[i+2][j+2] * grid[i+3][j+3]
                if product > answer:
                    answer = product
